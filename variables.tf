...
variable "project_id" {
  description = "STACKIT project ID to which the Redis instance and its credentials are associated."
  type        = string
}
...
variable "create_instance" {
  description = "Whether to create the Redis instance. Set to false to manage credentials against an existing instance provided via `instance_id`."
  type        = bool
  default     = true
}

variable "instance_id" {
  description = "ID of an existing Redis instance. Used for credentials when `create_instance` is false."
  type        = string
  default     = null
}
...
variable "redis_version" {
  description = "The Redis service version, e.g. \"7.2\"."
  type        = string
  default     = null
}
